...
(spec/def :records/operation-id int?)
(spec/def :records/user-id string?)
(spec/def :records/amount float?)
(spec/def :records/user-balance float?)
(spec/def :records/operation-response string?)
(spec/def :records/deleted boolean?)

(spec/def :records/create-record!
  (spec/keys :req-un [:records/operation-id
                      :records/user-id
                      :records/amount
                      :records/user-balance]))

(defn create-record!
  "Create a new record with the provided data. Returns 1 on success."
  [record-data]
  (-> record-data
    (assoc :operation_response "") ; Add default value for operation_response
